...
static void ntfs_init(void) {
  terminal_writestring("NTFS: Detected (read-only support not yet implemented)\n");
  terminal_writestring("NTFS: Falling back to RAMFS\n");
}

static int ntfs_read_file(const char *filename, uint8_t *buffer, uint32_t max_len) {
  (void)filename;
  (void)buffer;
  (void)max_len;
...
static void ext4_init(void) {
  terminal_writestring("ext4: Detected (read-only support not yet implemented)\n");
  terminal_writestring("ext4: Falling back to RAMFS\n");
}

static int ext4_read_file(const char *filename, uint8_t *buffer, uint32_t max_len) {
  (void)filename;
  (void)buffer;
  (void)max_len;
  return -1; /* Not implemented */
